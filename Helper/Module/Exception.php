...
class Exception
{
    ...
    private \M2E\Temu\Helper\Module\Log $logHelper;
    private \M2E\Temu\Model\Log\System\Repository $logSystemRepository;

    public function __construct(
        \M2E\Temu\Model\Log\System\Repository $logSystemRepository,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \M2E\Temu\Helper\Module\Log $logHelper
    ) {
    ...
    public function process(\Throwable $throwable, array $context = []): void
    {
        ...
        $type = \M2E\Temu\Model\Log\System::TYPE_EXCEPTION;
        if ($throwable instanceof \M2E\Core\Model\Exception\Connection) {
            $type = \M2E\Temu\Model\Log\System::TYPE_EXCEPTION_CONNECTOR;
        }
    ...
    private function processFatal(array $error, string $traceInfo): void
    {
        try {
            ...
            if (isset($error['message']) && strpos($error['message'], 'Allowed memory size') !== false) {
                $this->systemLog(
                    \M2E\Temu\Model\Log\System::TYPE_FATAL_ERROR,
                    $class,
                    $error['message'],
    ...
    {
        $additionalData = $throwable instanceof \M2E\Core\Model\Exception ? $throwable->getAdditionalData() : [];
