define([
    ...
    window.SynchProgress = Class.create(Common, {
        ...
        runTask: function (taskTitle, taskUrl, taskCheckUrl, callBackWhenEnd) {
            ...
            var self = this;
            self.start(taskTitle, Temu.translator.translate('Preparing to start. Please wait ...'));
            ...
            new Ajax.Request(taskCheckUrl, {
                ...
                onSuccess: function (transport) {
                    ...
                    if (data.ajaxExpired && response.ajaxRedirect) {
                        ...
                        alert(Temu.translator.translate('Unauthorized! Please login again.'));
        ...
        printFinalMessage: function () {
            ...
            if (self.result == self.resultTypeError) {
                MessageObj.addError(str_replace(
                        '%url%',
                        Temu.url.get('logViewUrl'),
                        Temu.translator.translate('Shop synchronization was completed with errors. <a target="_blank" href="%url%">View Log</a> for the details.')
                ));
            } else if (self.result == self.resultTypeWarning) {
                MessageObj.addWarning(str_replace(
                        '%url%',
                        Temu.url.get('logViewUrl'),
                        Temu.translator.translate('Shop synchronization was completed with warnings. <a target="_blank" href="%url%">View Log</a> for the details.')
                ));
            } else {
                MessageObj.addSuccess(Temu.translator.translate('Shop synchronization was completed.'));
            }
