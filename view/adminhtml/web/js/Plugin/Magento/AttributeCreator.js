define([
    ...
    window.AttributeCreator = Class.create({
        ...
        showPopup: function (params) {
            ...
            new Ajax.Request(Temu.url.get('general/getCreateAttributeHtmlPopup'), {
                method: 'post',
                asynchronous: true,
                parameters: params,
                onSuccess: function (transport) {
                    ...
                    self.popupObj = jQuery(modalDialogMessage).modal({
                        title: Temu.translator.translate('Creation of New Magento Attribute'),
                        type: 'slide',
                        buttons: [
                            {
        ...
        create: function (attributeParams) {
            ...
            new Ajax.Request(Temu.url.get('general/createAttribute'), {
                method: 'post',
                asynchronous: true,
        ...
        defaultOnSuccessCallback: function (attributeParams, result) {
            MessagesObj.addSuccess(Temu.translator.translate('Attribute has been created.'));
        ...
        chooseNewlyCreatedAttribute: function (attributeParams, result) {
            ...
            var existedOptionsCollection = self.haveOptgroup() ? $$('select[id="' + self.selectObj.id + '"] optgroup.Temu-custom-attribute-optgroup option')
        ...
        getNewlyCreatedAttributeValue: function (attributeParams) {
            ...
            var optGroupObj = $$('select[id="' + this.selectObj.id + '"] optgroup.Temu-custom-attribute-optgroup').first();

            if (optGroupObj.hasAttribute('new_option_value')) {
                return optGroupObj.getAttribute('new_option_value');
            }

            return $$('select[id="' + this.selectObj.id + '"] optgroup.Temu-custom-attribute-optgroup option').first().value;
        },
        ...
        injectAddOption: function () {
            ...
            }).update(Temu.translator.translate('Create a New One...'));

            if (self.haveOptgroup()) {
                $$('select[id="' + this.selectObj.id + '"] optgroup.Temu-custom-attribute-optgroup').first().appendChild(option);
            } else {
        ...
        validateAttributeCodeToBeUnique: function (value, el) {
            ...
            new Ajax.Request(Temu.url.get('general/isAttributeCodeUnique'), {
                method: 'post',
                asynchronous: false,
                parameters: {
                    code: value
        ...
        updateCode: function (label) {
            new Ajax.Request(Temu.url.get('general/generateAttributeCodeByLabel'), {
                method: 'post',
                asynchronous: true,
                parameters: {
                    store_label: label
        ...
        haveOptgroup: function () {
            var obj = $$('select[id="' + this.selectObj.id + '"] optgroup.Temu-custom-attribute-optgroup').first();
