define([
    ...
    window.MovingFromListing = Class.create(Action, {
        ...
        moveToListingGrid: function () {
            let self = this;
            let ignoreListings = JSON.parse(Temu.customData.ignoreListings);
            $.ajax(
                    {
                        url: Temu.url.get('moveToListingGridHtml'),
                        type: 'GET',
                        data: {
                        ...
                        success: function (response) {
                            self.completeProgressBar();
                            self.openPopUp(response, $t('Moving Temu Items'));
                        },
                    }
            );
        },
        ...
        submit: function (listingId, onSuccess) {
            ...
            $.ajax(
                    {
                        url: Temu.url.get('moveToListing'),
                        type: 'POST',
                        data: {
                            listingId: listingId
                        },
