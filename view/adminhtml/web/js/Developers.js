define([
    ...
    window.Developers = Class.create({
        ...
        saveDirectDatabaseChanges: function () {
            ...
            new Ajax.Request(Temu.url.get('developers/save'), {
                method: 'post',
                asynchronous: true,
                parameters: {
                ...
                onSuccess: function (transport) {
                    ...
                    if (result.success) {
                        MessagesObj.addSuccess(Temu.translator.translate('Settings saved'));
                        $('save_inspector_mode').hide();
                        self.inspectionModeValue = self.inspectionModeElement.value;
                    } else {
                        MessagesObj.addError(Temu.translator.translate('Error'));
                    }
                }
            });
        }
