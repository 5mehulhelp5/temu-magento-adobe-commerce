...
use M2E\Temu\Model\Order\Exception\ProductCreationDisabled;
...
class Reserve
{
    ...
    public const MAGENTO_RESERVATION_PLACED_EVENT_TYPE = 'temu_reservation_placed';
    public const MAGENTO_RESERVATION_RELEASED_EVENT_TYPE = 'temu_reservation_released';
    public const MAGENTO_RESERVATION_OBJECT_TYPE = 'temu_order';
    ...
    private \M2E\Temu\Model\Magento\Product\StockItemFactory $magentoProductStockItemFactory;

    public function __construct(
        \M2E\Temu\Model\Magento\ProductFactory $magentoProductFactory,
        \M2E\Temu\Model\Magento\Product\StockItemFactory $magentoProductStockItemFactory,
    ...
    {
        if ($this->isPlaced()) {
            throw new \M2E\Temu\Model\Exception\Logic('QTY is already reserved.');
        }
    ...
    {
        if ($this->isReleased()) {
            throw new \M2E\Temu\Model\Exception\Logic('QTY is already released.');
        }
    ...
    {
        if ($this->isCanceled()) {
            throw new \M2E\Temu\Model\Exception\Logic('QTY reserve is already canceled.');
        }
    ...
    public function addSuccessLogCancelQty(): void
    {
    ...
    private function getValidatedOrdersItems($action)
    {
        ...
        foreach ($this->order->getItems() as $item) {
            /**@var \M2E\Temu\Model\Order\Item $item */
        ...
        if ($productsExistCount === 0) {
            ...
            throw new \M2E\Temu\Model\Exception\Logic(
                'The product could not be found in the Magento catalog.'
            );
        }
    ...
    private function performAction($action, $newState)
    {
        ...
        foreach ($this->order->getItems() as $item) {
            ...
            foreach ($products as $productId => $productData) {
                /** @var \M2E\Temu\Model\Magento\Product $magentoProduct */
                /** @var \M2E\Temu\Model\Magento\Product\StockItem $magentoStockItem */
    ...
    protected function changeProductQty(
        ...
        \M2E\Temu\Model\Magento\Product\StockItem $magentoStockItem,
        $action,
        $qty,
    ...
    protected function changeMSIProductQty(
        ...
        $reservation = $this->objectManager->get(\M2E\Temu\Model\MSI\Order\Reserve::class);
    ...
    protected function pushQtyChangeInfo($qty, $action, \M2E\Temu\Model\Magento\Product $magentoProduct)
    {
        $this->qtyChangeInfo[] = [
            'action' => $action,
            'quantity' => $qty,
     ...
     */
    private function getItemProductsByAction(\M2E\Temu\Model\Order\Item $item, $action)
    {
        switch ($action) {
    ...
    private function isMsiMode(\M2E\Temu\Model\Magento\Product $product)
    {
