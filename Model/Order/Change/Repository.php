...
class Repository
{
    ...
    private \M2E\Temu\Model\ResourceModel\Order\Change\CollectionFactory $collectionFactory;
    private \M2E\Temu\Model\ResourceModel\Order $orderResource;

    public function __construct(
        OrderChangeResource $resource,
        \M2E\Temu\Model\ResourceModel\Order\Change\CollectionFactory $collectionFactory,
    ...
    public function create(\M2E\Temu\Model\Order\Change $change): void
    {
        $this->resource->save($change);
    }

    public function save(\M2E\Temu\Model\Order\Change $change): void
    {
        $this->resource->save($change);
    }

    public function delete(\M2E\Temu\Model\Order\Change $change): void
    {
        $this->resource->delete($change);
    }

    public function findExist(int $orderId, string $action, string $hash): ?\M2E\Temu\Model\Order\Change
    {
        ...
        /** @var \M2E\Temu\Model\Order\Change $change */
    ...
    /**
     * @return \M2E\Temu\Model\Order\Change[]
     */
    public function findShippingNotStarted(int $orderId): array
    {
        ...
        $collection
            ...
            ->addFieldToFilter(
                OrderChangeResource::COLUMN_ACTION,
                ['eq' => \M2E\Temu\Model\Order\Change::ACTION_UPDATE_SHIPPING]
            )
            ->addFieldToFilter(
     ...
     * @return \M2E\Temu\Model\Order\Change[]
     */
    public function findShippingReadyForProcess(\M2E\Temu\Model\Account $account, int $limit): array
    {
    ...
    private function findReadyForProcessByAction(
        ...
        $collection->joinInner(
            ...
            sprintf(
                ...
                \M2E\Temu\Model\ResourceModel\Order::COLUMN_ID,
                OrderChangeResource::COLUMN_ORDER_ID,
            ),
            [
                'account_id' => \M2E\Temu\Model\ResourceModel\Order::COLUMN_ACCOUNT_ID,
            ],
        );
        $collection->addFieldToFilter(
            sprintf('orders.%s', \M2E\Temu\Model\ResourceModel\Order::COLUMN_ACCOUNT_ID),
            ['eq' => $account->getId()],
        );
