...
namespace M2E\Temu\Model\ResourceModel\UnmanagedProduct\Grid;
    ...
    \Magento\Framework\Api\Search\SearchResultInterface
{
    ...
    private \M2E\Temu\Model\ResourceModel\Product\CollectionFactory $productCollectionFactory;
    private \M2E\Temu\Model\ResourceModel\UnmanagedProduct\VariantSku $variantSkuResource;

    public function __construct(
        \M2E\Temu\Model\ResourceModel\UnmanagedProduct\VariantSku $variantSkuResource,
        ProductResource $productResource,
        \M2E\Temu\Model\ResourceModel\Product\CollectionFactory $productCollectionFactory,
    ...
    private function buildSkuIdFilter($condition): void
    {
        $this->getSelect()->where(
            sprintf(
                ...
                \M2E\Temu\Model\ResourceModel\UnmanagedProduct::COLUMN_IS_SIMPLE
            ),
            $condition['like']
        );
    }
    ...
    private function buildQtyFilter($condition): void
    {
        ...
        if (isset($condition['gteq']) && $condition['gteq'] !== '') {
            $where .= sprintf(
                'main_table.%s >= %s',
                \M2E\Temu\Model\ResourceModel\UnmanagedProduct::COLUMN_QTY,
    ...
    private function buildFilterByLinked($condition): void
    {
        ...
        $column = \M2E\Temu\Model\ResourceModel\UnmanagedProduct::COLUMN_MAGENTO_PRODUCT_ID;

        if ($conditionValue === \M2E\Temu\Ui\Select\YesNoAnyOption::OPTION_YES) {
            $this->getSelect()->where(sprintf('main_table.%s IS NOT NULL', $column));
        } elseif ($conditionValue === \M2E\Temu\Ui\Select\YesNoAnyOption::OPTION_NO) {
