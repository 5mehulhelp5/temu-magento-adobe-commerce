...
class Runner
{
    ...
    private \M2E\Temu\Model\Connector\Client\Single $connector;
    ...
    public function __construct(
        \M2E\Temu\Model\Processing\CreateService $processingCreate,
        \M2E\Temu\Model\Connector\Client\Single $connector,
        \M2E\Temu\Model\Processing\ResultHandlerCollection $resultHandlerCollection,
        \M2E\Temu\Model\Processing\LockManagerFactory $lockManagerFactory
    ) {
    ...
    private function validateHandler(SingleInitiatorInterface $initiator): void
    {
        ...
        if ($initiator instanceof \M2E\Temu\Model\Processing\PartialInitiatorInterface) {
            if (!is_a($handlerClass, \M2E\Temu\Model\Processing\PartialResultHandlerInterface::class, true)) {
                throw new \M2E\Temu\Model\Exception\Logic('Result handler is not valid for this processing.');
            }

            return;
        }

        if (!is_a($handlerClass, \M2E\Temu\Model\Processing\SingleResultHandlerInterface::class, true)) {
            throw new \M2E\Temu\Model\Exception\Logic('Result handler is not valid for this processing.');
        }
    }
