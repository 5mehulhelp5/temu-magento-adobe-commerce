...
class Builder
{
    ...
    protected \M2E\Temu\Model\Order\ProxyObject $proxyOrder;
    ...
    /** @var \M2E\Temu\Model\Magento\Quote\ItemFactory */
    private ItemFactory $magentoQuoteItemFactory;

    public function __construct(
        \M2E\Temu\Model\Magento\Quote\Store\ConfiguratorFactory $configuratorFactory,
        ...
        \M2E\Temu\Model\Magento\Quote\ItemFactory $magentoQuoteItemFactory
    ) {
    ...
    private function initializeQuote()
    {
        ...
        $this->quote->setIsTemuQuote(true);
    ...
    private function initializeAddresses()
    {
        ...
        $billingAddress->setLimitCarrier('temushipping');
        $billingAddress->setShippingMethod('temushipping_temushipping');
        ...
        $shippingAddress->setLimitCarrier('temushipping');
        $shippingAddress->setShippingMethod('temushipping_temushipping');
    ...
    /**
     * @param \M2E\Temu\Model\Order\Item\ProxyObject $item
     * @param \M2E\Temu\Model\Magento\Quote\Item $quoteItemBuilder
    ...
    protected function initializeQuoteItem($item, $quoteItemBuilder, $product, $request)
    {
        ...
        if (is_string($result)) {
            throw new \M2E\Temu\Model\Exception($result);
        }
    ...
    {
        $this->quote->setUseTemuDiscount(false);
