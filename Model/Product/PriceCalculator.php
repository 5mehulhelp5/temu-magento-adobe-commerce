...
use M2E\Temu\Model\Magento\Product\Cache;
...
class PriceCalculator
{
    ...
    private array $sourceModeMapping = [
        self::MODE_NONE => \M2E\Temu\Model\Policy\SellingFormat::PRICE_MODE_NONE,
        self::MODE_PRODUCT => \M2E\Temu\Model\Policy\SellingFormat::PRICE_MODE_PRODUCT,
        self::MODE_SPECIAL => \M2E\Temu\Model\Policy\SellingFormat::PRICE_MODE_SPECIAL,
        self::MODE_ATTRIBUTE => \M2E\Temu\Model\Policy\SellingFormat::PRICE_MODE_ATTRIBUTE,
        self::MODE_TIER => \M2E\Temu\Model\Policy\SellingFormat::PRICE_MODE_TIER,
    ];
    ...
    /** @var array */
    private $modifier = [];
    /** @var \M2E\Temu\Model\Magento\Product\Cache|null */
    ...
    private \M2E\Temu\Helper\Magento\Attribute $attributeHelper;
    ...
    public function __construct(
        ...
        \M2E\Temu\Helper\Magento\Attribute $attributeHelper
    ) {
    ...
    private function getSourceMode()
    {
        if (!in_array($this->getSource('mode'), $this->sourceModeMapping)) {
            throw new \M2E\Temu\Model\Exception\Logic('Unknown source mode.');
        }
    ...
    /**
     * @param \M2E\Temu\Model\Magento\Product\Cache|null $attributeSourceProduct
     *
     * @return \M2E\Temu\Model\Product\PriceCalculator
     */
    ...
    /**
     * @return \M2E\Temu\Model\Magento\Product\Cache|null
     */
    ...
    //@todo Refactoring is strongly recommended
    private function getProductBaseValue()
    {
        ...
        if ($this->isSourceModeProduct()) {
            if ($this->getMagentoProduct()->isConfigurableType()) {
                ...
                $value = $this->convertValueFromStoreToSite($this->getMagentoProduct()->getPrice());
            }
            ...
            if ($this->getMagentoProduct()->isConfigurableType()) {
                if (
                    ...
                    $value = $this->attributeHelper->convertAttributeTypePriceFromStoreToSite(
    ...
    private function getExistedProductSpecialValue(Product $product): float
    {
        ...
        if ($value <= 0) {
            return $this->convertValueFromStoreToSite($product->getPrice());
        }

        return $this->convertValueFromStoreToSite($value);
    }
    ...
    private function getExistedProductTierValue(Product $product)
    {
        ...
        foreach ($tierPrice as $qty => $value) {
            $tierPrice[$qty] = $this->convertValueFromStoreToSite($value);
        }
    ...
    /**
     * @param \M2E\Temu\Model\Magento\Product $product
     *
     * @return double
    ...
    private function getGroupedTierValue(\M2E\Temu\Model\Magento\Product $product)
    {
    ...
    private function getBundleTierFixedValue(\M2E\Temu\Model\Magento\Product $product)
    {
        return $this->calculateBundleTierValue($product, $product->getPrice());
    }

    private function getBundleTierDynamicValue(\M2E\Temu\Model\Magento\Product $product)
    {
    ...
    private function convertValueFromStoreToSite($value)
    {
    ...
    private function getCurrencyForPriceConvert(): string
    {
        return $this->getListing()->getAccount()->getCurrencyCode();
    }
