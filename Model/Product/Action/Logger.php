...
class Logger
{
    ...
    protected function initLogType(\M2E\Core\Model\Response\Message $message): int
    {
        if ($message->isError()) {
            ...
            return \M2E\Temu\Model\Log\AbstractModel::TYPE_ERROR;
        }

        if ($message->isWarning()) {
            $this->setStatus(\M2E\Core\Helper\Data::STATUS_WARNING);

            return \M2E\Temu\Model\Log\AbstractModel::TYPE_WARNING;
        }

        if ($message->isSuccess()) {
            $this->setStatus(\M2E\Core\Helper\Data::STATUS_SUCCESS);

            return \M2E\Temu\Model\Log\AbstractModel::TYPE_SUCCESS;
        }

        if ($message->isNotice()) {
            $this->setStatus(\M2E\Core\Helper\Data::STATUS_SUCCESS);

            return \M2E\Temu\Model\Log\AbstractModel::TYPE_INFO;
        }
