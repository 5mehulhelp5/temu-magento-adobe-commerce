...
class Clearing
{
    ...
    private \M2E\Temu\Model\Synchronization\Log\Repository $syncLogRepository;
    ...
    public function __construct(
        ...
        \M2E\Temu\Model\Synchronization\Log\Repository $syncLogRepository,
    ...
    public function saveSettings(string $log, bool $mode, int $days): void
    {
        ...
        $this->config->set('/logs/clearing/' . $log . '/', 'mode', (int)$mode);
        $this->config->set('/logs/clearing/' . $log . '/', 'days', $days);
    }
    ...
    public function clearOldRecords(string $logType): void
    {
        ...
        $mode = (int)$this->config->get('/logs/clearing/' . $logType . '/', 'mode');
        $days = (int)$this->config->get('/logs/clearing/' . $logType . '/', 'days');
