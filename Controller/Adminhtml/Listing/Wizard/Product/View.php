...
class View extends \M2E\Temu\Controller\Adminhtml\Listing\Wizard\StepAbstract
{
    ...
    private \M2E\Temu\Model\Magento\Product\RuleFactory $magentoProductRuleFactory;
    ...
    public function __construct(
        ...
        \M2E\Temu\Model\Magento\Product\RuleFactory $magentoProductRuleFactory,
    ...
    protected function process(\M2E\Temu\Model\Listing $listing)
    {
        ...
        if ($source === \M2E\Temu\Block\Adminhtml\Listing\Wizard\ProductSourceSelect::MODE_PRODUCT) {
        ...
        if ($source === \M2E\Temu\Block\Adminhtml\Listing\Wizard\ProductSourceSelect::MODE_CATEGORY) {
    ...
    private function showGridByCatalog(\M2E\Temu\Model\Listing $listing, string $source)
    {
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            $this->setAjaxContent(
                $this->getLayout()
                     ->createBlock(
                         \M2E\Temu\Block\Adminhtml\Listing\Wizard\Product\Add\Grid::class,
                     )
                     ->toHtml(),
            );
        ...
        $this->addContent(
            $this->getLayout()->createBlock(
                \M2E\Temu\Block\Adminhtml\Listing\Wizard\Product\Add::class,
                '',
                [
                    'sourceMode' => $source,
                ],
    ...
    private function showGridByCategories(\M2E\Temu\Model\Listing $listing, string $source)
    {
        $this->setRuleData('product_add_step_one', $listing);
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            /** @var \M2E\Temu\Block\Adminhtml\Listing\Wizard\Product\Add\Category\Grid $grid */
            $grid = $this->getLayout()
                         ->createBlock(
                             \M2E\Temu\Block\Adminhtml\Listing\Wizard\Product\Add\Category\Grid::class,
                         );
        ...
        /** @var \M2E\Temu\Block\Adminhtml\Listing\Wizard\Product\Add $gridContainer */
        ...
        if (empty($data['current_category_id'])) {
            ...
            if (!$currentNode) {
                throw new \M2E\Temu\Model\Exception('No Categories found');
            }
    ...
    private function setRuleData(string $prefix, \M2E\Temu\Model\Listing $listing): void
    {
        ...
        $this->globalDataHelper->setValue(
            'rule_prefix',
            $prefix,
        ); // \M2E\Temu\Block\Adminhtml\Magento\Product\Grid:361
        ...
        $this->globalDataHelper->setValue(
            'rule_model',
            $ruleModel,
        ); // \M2E\Temu\Block\Adminhtml\Magento\Product\Grid:69
    }
}
