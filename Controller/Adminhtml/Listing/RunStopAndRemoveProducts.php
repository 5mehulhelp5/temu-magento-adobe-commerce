...
class RunStopAndRemoveProducts extends \M2E\Temu\Controller\Adminhtml\Listing\AbstractAction
{
    ...
    public function execute()
    {
        ...
        if ($this->isRealtimeProcessFromOldGrid()) {
            ['result' => $resultStatus, 'action_id' => $logsActionId] = $this->actionService->runStopAndRemove(
                $products,
            );
        } else {
            ['result' => $resultStatus, 'action_id' => $logsActionId] = $this->actionService->scheduleStopAndRemove(
                $products,
            );
        }
