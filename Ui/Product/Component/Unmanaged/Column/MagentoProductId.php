        ...
        foreach ($dataSource['data']['items'] as &$row) {
            ...
            if ($magentoProductId === null) {
                $link = sprintf(
                    ...
                    (int)$row['id'],
                    $productTitle,
                    $this->urlHelper->getUnmanagedProductPopupUrl(['account_id' => (int)$row['account_id']]),
                    __('Link')
                );
                ...
                $move = sprintf(
                    ...
                    (int)$row['id'],
                    (int)$row['id'],
                    $this->urlHelper->getUnmanagedPrepareMoveToListingUrl(),
                    $this->urlHelper->getUnmanagedMoveToListingUrl(),
                    $this->urlBuilder->getUrl('m2e_temu/listing_create/index'),
                    __('Move')
                );
                $html .= <<<HTML
