...
abstract class AbstractGrid extends \M2E\Temu\Block\Adminhtml\Log\Listing\AbstractGrid
{
    ...
    /**
     * @param \M2E\Temu\Model\Listing\Log|\Magento\Framework\DataObject $type DataObject for grouped mode
     *
     * @return int
     */
    ...
    protected function applyFilters(\Magento\Framework\Data\Collection $collection)
    {
        // Set listing filter
        // ---------------------------------------
        if ($this->getEntityId()) {
    ...
    public function callbackColumnListingTitleID($value, $row, $column, $isExport)
    {
        ...
        $manageUrl = $this->getUrl('*/listing/view', $urlData);
     ...
     */
    abstract protected function getExcludedActionTitles(): array;

    // ---------------------------------------

    protected function getActionTitles(): array
    {
        $allActions = \M2E\Temu\Helper\Module\Log::getActionsTitlesByClass(
