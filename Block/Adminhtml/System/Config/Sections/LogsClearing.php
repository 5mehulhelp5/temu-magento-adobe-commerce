...
use M2E\Temu\Model\Log\Clearing as LogClearing;

class LogsClearing extends \M2E\Temu\Block\Adminhtml\System\Config\Sections
{
    /** @var array */
    private $modes;
    /** @var array */
    private $days;
    private \M2E\Temu\Helper\Data $dataHelper;
    ...
    protected function _prepareForm()
    {
        ...
        $form->addField(
            ...
            self::HELP_BLOCK,
            [
                'content' => __('Set preferences for automatic clearing of Log data, then ' .
                    'click <strong>Save</strong>.You may clear the relevant logs manually by clicking ' .
...
HTML
        ...
        $fieldSet->addField(
            ...
            [
                ...
                'class' => 'Temu-logs-clearing-interval',
                'required' => true,
                'tooltip' => __(
...
HTML
    ...
    protected function prepareFormData(): void
    {
        ...
        foreach ($tasks as $task) {
            $modes[$task] = $config->get('/logs/clearing/' . $task . '/', 'mode');
            $days[$task] = $config->get('/logs/clearing/' . $task . '/', 'days');
        }
    ...
    protected function _prepareLayout(): void
    {
        ...
        $this->jsUrl->add($this->getUrl('m2e_temu/settings_logsClearing/save'), self::SECTION_ID_LOGS_CLEARING);
        $this->jsUrl->add($this->getUrl('m2e_temu/settings_logsClearing/save'), 'formSubmit');
